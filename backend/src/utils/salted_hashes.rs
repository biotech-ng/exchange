...
pub fn generate_hash_and_salt_for_text<T: AsRef<str>>(password: T) -> (String, String) {
    ...
    (hash_password(password, &salt_bytes), BASE_64.encode(salt_bytes))
}

pub fn generate_b64_hash_for_text_and_salt<T1: AsRef<str>, T2: AsRef<str>>(test: T1, salt_b64: T2) -> Result<String, DecodeError> {
