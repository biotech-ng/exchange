...
impl TokenResponse {
    ...
    pub fn new(user: UserInfo) -> Result<Self, CreateAccessTokenError> {
        let duration_in_secs = std::env::var("TOKEN_DURATION_IN_SECS").expect("SECRET_SALT must be in environment").parse::<i64>().expect("integer duration");
        ...
        let token = serde_json::to_string(&digest_access_token).map_err(CreateAccessTokenError::JsonError)?;

        Ok(Self { token: BASE_64.encode(token), expires_at, refresh_at, })
    }
...
impl AccessToken {
    fn new_with_user(user: UserInfo, duration: Duration) -> Self {
        ...
        Self { user, expires_at, refresh_at }
    }

    pub fn from_token<T: AsRef<str>>(token_b64: T) -> Result<Self, ParseAccessTokenError> {
        let token_bytes = BASE_64.decode(token_b64.as_ref()).map_err(ParseAccessTokenError::Base64DecodeError)?;

        let token = String::from_utf8(token_bytes).map_err(ParseAccessTokenError::StringDecodingError)?;

        let digest_token: DigestAccessToken = serde_json::from_str(&token).map_err(ParseAccessTokenError::ParseJsonError)?;

        let access_token: AccessToken = serde_json::from_str(digest_token.token.as_ref()).map_err(ParseAccessTokenError::ParseJsonError)?;
        let user = access_token.user.clone();
        let expires_at = access_token.expires_at;
        let refresh_at = access_token.refresh_at;

        let given_digest_access_token: DigestAccessToken = access_token.try_into().map_err(ParseAccessTokenError::CreateAccessTokenError)?;

        if given_digest_access_token.digest != digest_token.digest {
            return Err(ParseAccessTokenError::InvalidDigest);
        }

        Ok(AccessToken { user, expires_at, refresh_at, })
    }
...
impl TryFrom<AccessToken> for DigestAccessToken {
    ...
    fn try_from(value: AccessToken) -> Result<Self, Self::Error> {
        ...
        let digest = generate_b64_hash_for_text_and_salt(&token, salt_b64).map_err(CreateAccessTokenError::DecodeError)?;
...
mod tests {
    ...
    fn test_create_and_parse_a_valid_token() {
        ...
        let user = UserInfo { first_name, last_name, };
    ...
    fn test_parse_invalid_token() {
        ...
        let digest_access_token = DigestAccessToken {
            ...
            token: serde_json::to_string(&access_token).map_err(CreateAccessTokenError::JsonError).expect("valid access token"),
            expires_at,
            refresh_at,
        };

        let token = serde_json::to_string(&digest_access_token).map_err(CreateAccessTokenError::JsonError).expect("???");

        let token_response = TokenResponse { token, expires_at, refresh_at, };
